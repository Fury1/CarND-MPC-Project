 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int i = 0; i < N - 1; ++i) {
      ...
      fg[0] += 75 * CppAD::pow(vars[a_start], 2);
   }

    // Minimize the value gap between sequential actuations
    for (int i = 0; i < N - 2; ++i) {
      fg[0] += 300 * CppAD::pow(vars[delta_start + i + 1] - vars[delta_start + i], 2);
      fg[0] += 300 * CppAD::pow(vars[a_start + i +1] - vars[a_start], 2);
    }
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // Take the value one timestep into future to model latency, 100ms latency = dt of 0.1
